...
@interface KSCCameraViewController (/* Private */) <KSCLiveScannerDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate>

@property (nonatomic, strong, readwrite) KSCLiveScanner* liveScanner;
@property (nonatomic, strong, readwrite) IBOutlet UIView* previewView;
@property (nonatomic, strong, readwrite) IBOutlet KSCStatusView* cameraStatusView;
@property (nonatomic, strong, readwrite) IBOutlet KSCCameraZoomSlider* cameraZoomSlider;
@property (nonatomic, strong, readwrite) IBOutlet KSCCameraToolbar* cameraToolbar;
@property (nonatomic, strong, readwrite) IBOutlet KSCProgressToolbar* progressToolbar;
@property (nonatomic, strong, readwrite) IBOutlet KSCCameraModeControl* cameraModeControl;
...
@implementation KSCCameraViewController
...
- (KSCLiveScanner *)liveScanner
{
    if (!liveScanner) {
        liveScanner = [[KSCLiveScanner alloc] init];
...
- (instancetype)init
{
    ...
    [KSCCameraModeControl class];
    [KSCCameraToolbar class];
    [KSCCameraZoomSlider class];
    [KSCCustomToolbar class];
    [KSCCustomToolbarButton class];
    [KSCProgressToolbar class];
    
    
	self = [super initWithNibName:@"KSCCameraViewController" bundle:[KSCSDKConfig SDKBundle]];
...
{
    NSLog(@"KSCCameraViewController: custom nib ignored. Use init to instantiate an instance");
    return [self init];
}
...
- (void)viewDidLoad
{
	...
	KSCLiveScannerMode mode = kKSCLiveScannerLiveScanningMode;
	...
	{
		mode = kKSCLiveScannerSingleShotMode;
	}
...
- (IBAction)cameraModeChanged:(id)sender
{
	DebugLog(@"switching to mode: %@", self.liveScanner.liveScannerMode == kKSCCaptureSessionLiveScanningMode ? @"snapshot" : @"live");
	[self hideCameraHelp];
	
	if (self.cameraModeControl.cameraMode == kCameraModeLiveScanning && self.liveScanner.liveScannerMode == kKSCLiveScannerSingleShotMode)
	{
		[self switchToMode:kKSCLiveScannerLiveScanningMode];
		[self showStatusViewForModeStatusChange];
		[[NSUserDefaults standardUserDefaults] setBool:NO forKey:kUserPreferenceCameraStartsInSnapshotMode];
        [self startScanLineAnimation];
	}
	else if (self.cameraModeControl.cameraMode == kCameraModeSingleShot && self.liveScanner.liveScannerMode == kKSCLiveScannerLiveScanningMode)
	{
		[self switchToMode:kKSCLiveScannerSingleShotMode];
		[self showStatusViewForModeStatusChange];
...
#pragma mark - KSCLiveScannerDelegate

- (void)liveScanner:(KSCLiveScanner*)scanner recognizingImage:(NSData*)imageData
{
    if (self.liveScanner.liveScannerMode == kKSCLiveScannerSingleShotMode) {
